...
# Simulated provider latency / failure hooks for eval injection. Context-local
# storage prevents concurrent runs from injecting failures into one another.
_FAULT: ContextVar[tuple[bool, str | None]] = ContextVar("payment_adapter_fault", default=(False, None))


def set_fault(timeout: bool = False, error: str | None = None) -> None:
    _FAULT.set((timeout, error))
...
def quote(
    ...
    *,
    ...
    force_timeout, force_error = _FAULT.get()
    if force_timeout:
        time.sleep(0.05)
        return ToolEnvelope(
            ok=False,
