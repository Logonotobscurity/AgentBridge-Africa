...
_guardian = BudgetGuardian(emit_oscal=True)
_tracer = Tracer()
_hitl = HitlGate()
...
def _emit_oscal(state: BridgeState) -> None:
    ...
    findings = [
        ...
        Finding(
            ...
            success=state.status != "failed" or "offline" not in (state.stop_reason or ""),
            rationale=state.stop_reason or "policy satisfied",
            severity="moderate",
        ),
        Finding(
            control_id="AB-RUN-1",
...
def run_quote_goal(
    ...
    *,
    ...
    span = _tracer.start("run_quote_goal", attributes={"goal": goal, "locale": state.profile.locale})
    ...
    state = _guardian.charge(state, step.cost_usd)
    if state.status == "budget_exceeded":
        clear_fault()
        _tracer.end(span, status="ERROR", attributes={"http_status": 402})
        state.traces.append(span.to_record())
        return state
    ...
    if execute_payment:
        ...
        if ticket is not None and not hitl_approved:
            ...
            clear_fault()
            _tracer.end(span, status="UNSET", attributes={"hitl": ticket.ticket_id})
            state.traces.append(span.to_record())
            return state
        ...
        state = _guardian.charge(state, float(ex.cost_estimate or 0.0))
        if state.status == "budget_exceeded":
            clear_fault()
            _tracer.end(span, status="ERROR", attributes={"http_status": 402})
            state.traces.append(span.to_record())
...
def run_budget_exhaustion(profile: ContextProfile | None = None) -> BridgeState:
    ...
    for _ in range(5):
        state = _guardian.charge(state, 0.001)
        if state.status == "budget_exceeded":
            return state
    state.status = "failed"
